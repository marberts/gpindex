...
test_that("contributions work correctly", {
  ...
  expect_equal(
    sum(contributions(-3.75)(x, w)),
    generalized_mean(-3.75)(x, w) - 1
  )
  expect_equal(
    sum(contributions(3.75)(xna, w), na.rm = TRUE),
...
test_that("nested contributions work correctly", {
  ...
  expect_equal(
    fisher_contributions(1:2, c(NA, NA)),
  ...
  expect_equal(
    nested_contributions(3, c(-1, 2), c(0.75, NA))(x),
  ...
  expect_equal(
    nested_contributions(3, c(2, -1), c(NA, 0.75))(x, w2 = xna),
  ...
  expect_equal(
    nested_contributions(3, c(-1, 2), c(NA, NA))(x),
    rep(NA_real_, length(x))
  )

  expect_equal(
    nested_contributions2(3, c(-1, 2), c(NA, NA))(x),
    rep(NA_real_, length(x))
  )
})
