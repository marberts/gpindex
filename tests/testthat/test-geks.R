...
test_that("geks works in corner cases", {
  ...
  expect_equal(tornqvist_geks(1:2, 1:2, letters[1:2], c(1, 1)), list(c(b = 2)))
  expect_equal(
    tornqvist_geks(c(NA, 1), c(1, NA), letters[1:2], c(1, 1)),
    list(c(b = NaN))
  )
})
...
test_that("no balance", {
  ...
  ti <- balanced(geometric_index("Tornqvist"))(
    1:3,
    c(4, 5, NA),
    5:3,
    c(2, 1, NA),
  ...
  expect_equal(
    cumprod(
      as.numeric(
        unlist(with(
          dat2,
          jevons_geks(price, quantity, period, product, na.rm = TRUE)
        ))
      )
    ),
    c(
