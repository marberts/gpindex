...
test_that("grouping and balacing work", {
  ...
  expect_equal(balanced(fisher_mean)(c(1, NA, 3, 4), c(NA, 1, 1, 2), c(1, 2, NA, 4), na.rm = TRUE), 4)
})

test_that("error happen when expected", {
  expect_error(generalized_mean("a"))
  ...
  expect_error(extended_mean("a", mtcars))
