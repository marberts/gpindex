...
all.equal(arithmetic_mean(1:4, scale = FALSE), 10)
all.equal(arithmetic_mean(1:4, 4:1, scale = FALSE), 20)
...
# NAs
all.equal(arithmetic_mean(NA, 0.5, na.rm = TRUE, scale = FALSE), 0)
...
all.equal(arithmetic_mean(1:2, c(2, NA), na.rm = TRUE, scale = FALSE), 2)
all.equal(arithmetic_mean(c(1, NA), c(NA, 2), na.rm = TRUE, scale = FALSE), 0)
# Zero values
