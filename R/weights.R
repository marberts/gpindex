index_weights <- function (p1, p0, q1, q0, pb, qb, type) {
  ...
  switch(type,
         ...
         Jevons = rep.int(1, length(p0)),
         Tornqvist = 0.5 * p0 * q0 / sum(p0 * q0) + 0.5 * p1 * q1 / sum(p1 * q1),
         Vartia1 = logmean(p0 * q0, p1 * q1) / logmean(sum(p0 * q1), sum(p1 * q1)),
         MontgomeryVartia = logmean(p0 * q0, p1 * q1) / logmean(sum(p0 * q1), sum(p1 * q1)),
