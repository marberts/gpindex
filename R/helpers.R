...
check_weights_arguments <- function(p1, p0, q1, q0, type) {
  stopifnot(
    ...
    "q1 must be numeric" = missing(q1) || is.numeric(q1),
    "q0 must be numeric" = missing(q0) || is.numeric(q0),
    "p1 and p0 must be the same length" = (missing(p1) || missing(p0)) || (length(p0) == length(p1)),
    "q1 must be the same length as p1 and p0" = missing(q1) || ((missing(p0) || length(p0) == length(q1)) && (missing(p1) || length(p1) == length(q1))),
    "q0 must be the same length as p1 and p0" = missing(q0) || ((missing(p0) || length(p0) == length(q0)) && (missing(p1) || length(p1) == length(q0)))
  )
}