...
### Transitivity and monotonicity properties
...
\mathbf{v}(\mathbf{r}, \mathbf{v}(\mathbf{r}, \mathbf{w}; \rho, \varsigma); \tau, \rho) \equiv \mathbf{v}(\mathbf{r}, \mathbf{w}; \rho, \varsigma).
...
The decomposition given by @eq-res1 is unique when $n = 2$ and $r_{1} \neq r_{2}$, and is the only such function that always returns $\mathbf{w}$ when $r_{1} = r_{2}$.[^4] But @eq-res1 is necessarily not unique when $n\geq3$, and there are infinitely many ways to decompose an index based on the generalized mean.[^5]
...
The decomposition in @eq-res3 can be extended to get a decomposition for an index constructed using the usual two-step procedure to make a price index. If price relatives are partitioned into $n$ groups, and an index is made from $n$ sub-indexes $\mathbf{m} = \left(\mathfrak{M}_{\rho_1}(\mathbf{r_{1}}, \mathbf{w}_1), \ldots, \mathfrak{M}_{\rho_n}(\mathbf{r_{n}}, \mathbf{w}_n)\right)$ with weights $(\omega_{1},\ldots,\omega_{n})$ as a generalized mean of order $\rho$, then

$$
v_{i}(\mathbf{m}, \mathbf{\omega}; \rho, \varsigma) \mathbf{v}(\mathbf{r}_{i}, \mathbf{w}_i; \rho_1, \varsigma)
$$
...
Any price index that can be represented as a generalized mean (of any order) admits an addition decomposition, and therefore automatically admits a percent change contribution, $v_i(\mathbf{r}, \mathbf{w}; \rho, 1) (r_i - 1)$, that sum up to $\mathfrak{M}_{\rho}(\mathbf{r}, \mathbf{w}) - 1$. The converse is not true and it is possible to decompose an index into percent-change contributions that do not allow it to be represented as an arithmetic mean. For example, consider the following "weights" to construct percent-change contributions

$$
\tilde{v}_i(\mathbf{r}, \mathbf{w}; \rho, 1) = 
\begin{cases}
(\mathfrak{M}_{\rho}(\mathbf{r}, \mathbf{w}) - 1)/(r_{i} - 1) & \text{if } r \neq 1, \\
0 & \text{if } r \neq 0.
\end{cases}
$$

Although $\sum_{i=1}^{n} \tilde{v}_{i}(\mathbf{r}, \mathbf{w}; \rho, 1) (r_{i} - 1) \equiv \mathfrak{M}_{\rho}(\mathbf{r}, \mathbf{w}) - 1$,
