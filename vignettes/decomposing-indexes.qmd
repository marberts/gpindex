...
Second, the extended mean is a strictly increasing function  in both arguments [@bullen2003, theorem 16]. This means that the function in @eq-res1 has a monotonicity property where the weights increase (decrease) for large (small) price relatives if and only if $\rho > \varsigma$. That is, if $\rho > \varsigma$, then there is a value $\bar{r} \in (\min(\mathbf{r}), \max(\mathbf{r}))$ such that $v_{i}(\mathbf{r}, \mathbf{w}; \rho, \varsigma)\geq w_{i}$ for all $i$ such that $r_{i} \geq \bar{r}$ and $v_{i}(\mathbf{r}, \mathbf{w}; \rho, \varsigma) < w_{i}$ for all $i$ such that $r_{i} < \bar{r}$, with these equalities reversed if $\rho < \varsigma$.^[To see this, note that $v_{i}(\mathbf{r}, \mathbf{w}; \rho, \varsigma) \geq w_{i}$ if and only if
$$
...
$$
Since the extended mean is strictly increasing in its first argument, and if $\rho > \varsigma$, then there is some $\bar{r}\in (\min(\mathbf{r}), \max(\mathbf{r}))$ such that this inequality is satisfied whenever $r\geq\bar{r}$. If $\rho < \varsigma$ then this inequality is satisfied whenever $r\leq\bar{r}$.]

## Decomposing superlative indexes

The additive and multiplicative decompositions for the Fisher index by @reinsdorf2002[section 6] can be generalized in the same way as the decompositions for the arithmetic and geometric indexes by noting that the Fisher index is simply a nested generalized mean of indexes based on the generalized mean. For a pair of generalized means $(\mathfrak{M}_{\rho_1}(\mathbf{r}, \mathbf{w}_1), \mathfrak{M}_{\rho_2}(\mathbf{r}, \mathbf{w}_2))$ mapping into $R_2^{++}$ with weights $(\omega_1, \omega_2) \in \Delta^1$, an index based on nested generalized means is written as 
$$
...
$$
The transformation in @eq-res1 then applies as before, just replacing $\mathbf{w}$ with the more complicated weights $\omega_1 \mathbf{v}(\mathbf{r}, \mathbf{w}_1; \rho_1, \rho) + \omega_2\mathbf{v}(\mathbf{r}, \mathbf{w}_2, \rho_2, \rho)$.
