...
@vonderlippe2015 discusses using the contra-harmonic mean, a special case of the Lehmer mean [@bullen2003, page 245], to make price indexes. The Lehmer mean of order $\rho$ can be written as $\mathfrak{M}_{\rho}(\mathbf{r}, \mathbf{w} \odot \mathbf{r}^{\rho - 1})$, and so indexes based on these means can be decomposed with @eq-res1 by using the weight $w_{i} r_{i}^{\rho - 1}$ for the $i$-th price relative instead of $w_{i}$.
