...
all.equal(
  v1,
  quadratic_weights(rel, nested_transmute(0, c(1, -1), 2)(rel, s1, s2))
)
```
...
all.equal(
  v2,
  quadratic_weights(rel, nested_transmute2(0, c(1, -1), 2)(rel, s1, s2))
)
```
...
$$
v_{i}(\mathfrak{N}(\mathbf{r}, \mathbf{w}_{1}, \ldots, \mathbf{w}_{n}), \mathbf{\omega}; \rho, \varsigma) \mathbf{v}(\mathbf{r}_{i}, \mathbf{w}_i; \rho_1, \varsigma)
$$
...
decomp_a <- quadratic_weights(rel_by_group$a, s1_by_group$a)
decomp_b <- transmute_weights(0, 1)(rel_by_group$b)

v <- Map(
  `*`,
  quadratic_weights(c(index_a, index_b), sapply(s1_by_group, sum)),
