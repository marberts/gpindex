...
Generalized means, and therefore generalized indexes, are calculated with the \texttt{generalized\_mean()} function in the \texttt{gpindex} package. This function takes a single number \texttt{r} and returns a function to compute the corresponding generalized mean. For example, \texttt{generalized\_mean(1)()} is the function to compute the usual arithmetic mean, and is mostly identical to the \texttt{weighted.mean()} function in the \texttt{stats} package, whereas \texttt{generalized\_mean(0)()} is the geometric mean, and \texttt{generalized\_mean(2)()} is the quadratic mean. Given their importance, the arithmetic, geometric, and harmonic means all have their own functions that simply call \texttt{generalized\_mean()} with \texttt{r} taking the values $1$, $0$, and $-1$. Although there are many implementations of \emph{unweighted} geometric and harmonic means for \textsf{R}, in most cases a \emph{weighted} mean is needed for calculating a price index.
...
Note that the weights returned by \texttt{index\_weights()} need not sum to 1, like all functions in \texttt{gpindex} that operate on weights, as this isn't always appropriate for some indexes (namely the Vartia-I index). The \texttt{scale\_weights()} function scales a numeric vector so that it sums to 1, so unscaled weights in general pose no special challenges, and \texttt{generalized\_mean()} scales the weights by default.
...
Although most named generalized indexes can be calculated by combining the appropriate names in \texttt{index\_weights()} with the appropriate order in \texttt{generalized\_mean()}, there are a number of wrappers to simplify doing this. For example, the \texttt{index\_arithmetic()} function takes the name of an arithmetic index and returns a function to calculate it, essentially passing the appropriate function to calculate the weights to \texttt{arithmetic\_mean()}.
...
The \texttt{gpindex} package has functions for calculating the generalized logarithmic mean and extended mean. The \texttt{generalized\_logmean()} function takes a parameter \texttt{r} and returns a function to calculate the corresponding generalized logarithmic mean. Similarly, \texttt{extended\_mean()} takes a pair of parameters \texttt{r} and \texttt{s} and returns the appropriate extended mean. The \texttt{transmute\_weights()} function wraps these up to calculate the weights to turn a generalized index of order $r$ into a generalized index of order $s$. (There are other uses for logarithmic means, like constructing the weights for the Vartia-I and Vartia-II indexes.)
...
Quote contributions for an index are easy to calculate by transmuting the weights for the index to arithmetic weights with \texttt{transmute\_weights(r, 1)()}. The \texttt{contributions()} function saves from having to do the manual calculation.
...
The \texttt{factor\_weights()} function takes a number \texttt{r} and returns a function to calculate the updated weights needed to chain an index of order \texttt{r}. The \texttt{update\_weights()} function covers the special case of \texttt{r = 1}.
