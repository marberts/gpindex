...
It is often useful to be able to decompose a price index into an additive contribution for each price relative. That is, for any number $r$, find a function $\tilde{w}_{r}$ of $p_{1} / p_{0}$ and $w$, such that $\sum_{i = 1}^{n} \tilde{w}_{ir}(p_1 / p_0, w) \equiv 1$ and
\begin{align*}
I_r(p_1 / p_0, w) \equiv \sum_{i = 1}^{n} \tilde{w}_{ir}(p_1 / p_0, w) p_1 / p_0.
\end{align*}
If such a function exists, it must be unique (unless all price relatives are equal).\footnote{Consider the case of $n = 2$, and suppose $p_{11} / p_{10} > p_{21} / p_{20}$. If there are numbers $w_{1}$ and $w_{2}$ such that $w_{1} p_{11} / p_{10} + (1 - w_{1}) p_{21} / p_{20} = w_{2} p_{11} / p_{10} + (1 - w_{2}) p_{21} / p_{20}$ then this implies that $w_{1} = w_{2}$.}
